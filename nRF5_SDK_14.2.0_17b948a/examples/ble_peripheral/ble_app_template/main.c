...
//uses different microphone than robot
#define MIC_CLK 16
#define MIC_LR  18
#define MIC_DI  17
...
//uses older version of motor board
#define SLEEP_L  0
#define M1_L     1
#define M0_L     2
#define DIR_L    3
...
int main(void)
{
    static uint8_t flipper = 0, lowhigh  = 0;
    int16_t i;
    uint32_t cnt, cnt2;
   ...
   // pwm_buzzer_frequency(2000);
    ...
    while (true)
    {
      nrf_delay_us(300);   //15ms*2=33Hz
      if (flipper == 0)
      {
          nrf_gpio_pin_clear(STEP_L);
          nrf_gpio_pin_clear(STEP_R);
          nrf_gpio_pin_clear(GREEN_LED);
          flipper = 1;
      }
      else  
      {
          nrf_gpio_pin_set(STEP_L);
          nrf_gpio_pin_set(STEP_R);
          nrf_gpio_pin_set(GREEN_LED);
          flipper = 0;
      }
      ++cnt;
...
void VLX6180_init(void)
{
    nrf_gpio_cfg_output(GP0);
    nrf_gpio_pin_clear(GP0);
    nrf_gpio_cfg_output(GP1);
    nrf_gpio_pin_clear(GP1);
...
void twi_init(void)
{
    ...
    err_code = nrf_drv_twi_init(&m_twi, &twi_VLX6180_config, twi_handler, NULL);
...
void configure_microphone(void)
{
  ...
  nrf_gpio_cfg_output(MIC_LR);
  nrf_gpio_pin_set(MIC_LR);   //Left, data on rising edge of clock
  NRF_PDM->MODE = 3;
...
void my_configure(void)
{
  ...
  NRF_PWM1->PSEL.OUT[0] = (STEP_R << PWM_PSEL_OUT_PIN_Pos) | (PWM_PSEL_OUT_CONNECT_Connected << PWM_PSEL_OUT_CONNECT_Pos);
  NRF_PWM1->PSEL.OUT[1] = (STEP_L << PWM_PSEL_OUT_PIN_Pos) | (PWM_PSEL_OUT_CONNECT_Connected << PWM_PSEL_OUT_CONNECT_Pos);
...
void configure_motors(void)
{
    nrf_gpio_cfg_output(STEP_L);
    nrf_gpio_cfg_output(DIR_L);
    nrf_gpio_cfg_output(M1_L);
    nrf_gpio_cfg_output(M0_L);
    nrf_gpio_cfg_output(SLEEP_L);

    nrf_gpio_cfg_output(STEP_R);
    nrf_gpio_cfg_output(DIR_R);
    nrf_gpio_cfg_output(M1_R);
    nrf_gpio_cfg_output(M0_R);
    nrf_gpio_cfg_output(SLEEP_R);
 
    nrf_gpio_pin_clear(STEP_L);
    nrf_gpio_pin_set(DIR_L);
    nrf_gpio_pin_set(M1_L);
    nrf_gpio_pin_set(M0_L);
    nrf_gpio_pin_set(SLEEP_L);

    nrf_gpio_pin_clear(STEP_R);
    nrf_gpio_pin_clear(DIR_R);
    nrf_gpio_pin_set(M1_R);
    nrf_gpio_pin_set(M0_R);
    nrf_gpio_pin_set(SLEEP_R);
}
#else
void configure_microphone(void)
{
  ...
  NRF_PDM->PSEL.CLK = MIC_CLK;
  NRF_PDM->PSEL.DIN = MIC_DI;
  nrf_gpio_cfg_output(WS);
  nrf_gpio_pin_set(WS);   //Left, data on rising edge of clock
  NRF_PDM->MODE = 3;
  //NRF_PDM->PDMCLKCTRL = 0x08800000;
  NRF_PDM->GAINL = 0;  
  NRF_PDM->GAINR = 0;
